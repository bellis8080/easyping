...
describe('Category Archive API Route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  const createMockRequest = () => {
    return new NextRequest(
      new URL('http://localhost/api/categories/cat-123/archive'),
      {
        method: 'POST',
      }
    );
  ...
  it('should return 400 if trying to archive default category', async () => {
    const mockUser = { id: 'user-123' };
    ...
    const mockAdminClient = {
      from: vi.fn().mockImplementation(() => {
        ...
        return createChainMock({
          data: { id: 'cat-123', is_default: true, is_active: true },
          error: null,
        });
      }),
    };
    ...
    expect(data.error).toContain('Cannot archive the default category');
  });

  it('should return 400 if category is already archived', async () => {
    const mockUser = { id: 'user-123' };
    ...
    const mockAdminClient = {
      from: vi.fn().mockImplementation(() => {
        ...
        return createChainMock({
          data: { id: 'cat-123', is_default: false, is_active: false },
          error: null,
        });
      }),
    };
